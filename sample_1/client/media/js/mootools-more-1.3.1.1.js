...
...
*/
//CHANGED BY ME (MAX OLIFER 8.07.2011)
var Slider = new Class({
	...
	initialize: function(element, knob, options){
		...
		var minLimitPos = this.toPosition(options.minLimit);
		limit[this.axis] = [-options.offset+minLimitPos, this.full - options.offset];
	...
	setHistory: function(lastknob,step){
		var position = this.toPosition(step);
		lastknob.setStyle(this.property, position);
	},
	attach: function(){
	...
	clickedElement: function(event){ 
		...
		//alert(newStep);
		if(this.options.minLimit<=newStep){
	...
	scrolledElement: function(event){
		...
		var newStep = this.step + (mode ? -1 : 1);
		if(this.options.minLimit<=newStep)
	...
	draggedKnob: function(){ 
		...
		position = position.limit(-this.options.offset, this.full -this.options.offset);
		//var newStep = this.toStep(position);
		//if(newStep>this.options.minLimit){
			this.step = Math.round(this.min + dir * this.toStep(position));
			this.checkStep();
		//}
	},
