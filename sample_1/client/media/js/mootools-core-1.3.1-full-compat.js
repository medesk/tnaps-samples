...
var IFrame = new Type('IFrame', function(){
	...
	delete props.onload;
	props.id = props.name = [props.id, props.name, iframe ? (iframe.id || iframe.name) : 'IFrame_' + String.uniqueID()].pick();/*MAX*/
	...
	id: (function(){
		...
		var types = {
			...
			element: function(el, nocash){
				...
				//Хак для IE 8, чтобы иметь доступ к нативному событию браузера через _fireEvent
				if (!nocash && !el.$family && !(/^object|embed$/i).test(el.tagName)){
...
Fx.Morph = new Class({
	...
	set: function(now){//alert(this.element.setStyle);
...
var Request = this.Request = new Class({
	...
	options: {/*
		...
		urlEncoded: true,
		encoding: 'utf-8',
		//Добавлена опция content_type, чтобы не использовать headers каждый раз
	...
	send: function(options){
		...
		if (this.options.urlEncoded && ['post', 'put'].contains(method)){
			...
			this.headers['Content-Type'] = this.options.content_type + encoding;
		}
		...
		if (this.options.noCache) {
			url += (url.contains('?') ? '&a=' : '?a=') + String.uniqueID();
		...
		if (this.options.timeout>0){ 
			...
			this.timeoutTimer=window.setTimeout(this.onTimeout.bindWithEvent(this), this.options.timeout);
