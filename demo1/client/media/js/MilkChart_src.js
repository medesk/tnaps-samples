...
MilkChart.Base = new Class({
    ...
    initialize: function(el, options) {
		...
		// Hacky, oh the shame!
        this.minY = (this.options.useZero) ? 1000 : 10000000000;
        this.maxY = 0;
    ...
    //	var val = 
        return this.ctx.measureText(String((this.maxY/1000).toFixed(4))).width;
    },
    ...
    drawValueLines: function() {
		...
		for (i = 0; i < this.chartLines; i++) {
			...
			if (this.options.showValues) {
				var offsetX = 8;
				var offsetY = 3
				this.ctx.fillText(String((lineValue/1000).toFixed(4)), this.bounds[0].x - offsetX, lineY + offsetY);
			}
    ...
    getData: function() {
        ...
        this.element.getElement('tbody').getChildren().each(function(row) {
            row.getChildren().each(function(node, index) {
                val = Number(node.get('html'))*1000;
                if (!$type(val)) {
                    var val = node.get('html').toFloat()*1000;
                }
